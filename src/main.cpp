...
#include "./sdr/sdr_driver.hpp"
#include "./sdr/sdr_main.hpp"
...
de::sdr::CSDRParser& cSDRParser = de::sdr::CSDRParser::getInstance();
...
void _version (void)
{
    std::cout << std::endl << _SUCCESS_CONSOLE_BOLD_TEXT_ "Drone-Engage Software-Defined Radio (SDR) Module version " << _INFO_CONSOLE_TEXT << version_string << _NORMAL_CONSOLE_TEXT_ << std::endl;
}
    ...
    try
    {
        ...
        if (std::strcmp(jMsg[INTERMODULE_ROUTING_TYPE].get<std::string>().c_str(), CMD_TYPE_INTERMODULE)==0)
        {
            ...
            if (messageType== TYPE_AndruavModule_ID)
            {
                ...
                cSDRMain.setPartyID(cModule.getPartyId(), cModule.getGroupId());
